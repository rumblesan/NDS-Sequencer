...
#define ENTRIES_PER_SCREEN 18
#define ENTRIES_START_ROW 4
...
void showDirectoryContents (const vector<DirEntry>& dirContents, int startRow)
{
	...
	if (strlen(path) < SCREEN_COLS) {
		iprintf ("\x1b[2;2H%s", path);
	} else {
		iprintf ("\x1b[2;2H%s", path + strlen(path) - SCREEN_COLS);
	}
	...
	for (int i = 0; i < ((int)dirContents.size() - startRow) && i < ENTRIES_PER_SCREEN; i++) {
		...
		// Set row
		iprintf ("\x1b[%d;2H", i + ENTRIES_START_ROW);
...
string browseForFile (const string& extension)
{
	...
	while (true) {
		...
		// Show cursor
		iprintf ("\x1b[%d;2H*", fileOffset - screenOffset + ENTRIES_START_ROW);
